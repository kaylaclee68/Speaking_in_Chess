...
# I/O
epochs = 5
out_dir = 'out_chess_llm_finetune_2'
eval_interval = 1000
log_interval = 1
eval_iters = 200
...
def make_batch_train_iterator():
    for batch in iter(train_loader):
        ...
        if seq_ins is None or seq_outs is None:
            continue

        max_prompt_tokens = len(tokenizer.encode(seq_ins[-1]))

        full_sequence = list(map(lambda t: "".join(t) + EOS, zip(seq_ins, seq_outs)))

        ids = torch.IntTensor([enc.ids for enc in tokenizer.encode_batch(full_sequence)]).type(torch.int64)
        X = ids[:,:-1]
        Y = ids[:,1:][:, max_prompt_tokens:]
        yield X.to(device), Y.to(device), max_prompt_tokens
    return
...
for epoch in range(epochs):
    ...
    def estimate_loss():
        ...
        for split in ['train']:
            ...
            for k in range(eval_iters):
                X, Y, prompt_len = get_batch()
                with ctx:
                    logits, loss = model(X, Y, mask_prompt_loss=True, prompt_len=prompt_len)
                losses[k] = loss.item()
            out[split] = losses.mean()
        model.train()
        return out
    X, Y, prompt_len = get_batch() # fetch the very first batch
    t0 = time.time()
    ...
    while True:
        ...
        if iter_num % log_interval == 0 and master_process:
            ...
            print(f"Epoch: {epoch}, iter {iter_num}: loss {lossf:.4f}, time {dt*1000:.2f}ms, mfu {running_mfu*100:.2f}%")
        iter_num += 1
        local_iter_num += 1
